    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if "turma" in k:
                ...
                matérias = formatarArray(calcularMatérias(db, usuario, turma=turma))
                ...
                try: 
                    ...
                    matéria = int(request.COOKIES["materia"]) if "materia" in k else 0
                    dados = d.calcularMediaTurma(f"{cursoturma.upper()} {serieturma}º ANO", "1º", matérias[matéria]) 
                except Exception as e:
                    dados = [0, 0, 0, 0]
                    matéria = 0
                return render(request, 'dashboard.html', {"dados":dados, "acertosClass":"chartAcertos-container" if "ultimaAvaliacao" in k and dados != [0, 0, 0, 0] else "chartAcertosDisabled-container", "turma":f"{cursoturma} {serieturma}º Ano", "matérias":matérias, "ind":matéria})
            else:
                return render(request, 'dashboard.html', {"dados":[20, 45, 49, 20], "labels":[f"{n}º" for n in range(1, 10+1)], "acertosClass":"chartAcertosDisabled-container", "turma":f"{cursoturma} {serieturma}º Ano", "matérias":["Sem matérias"], "ind":0})
        except Exception as e:
            return erro(4)
...
def calcularTurmas(db, usuario):
    ...
    for tt in db.Turmas.find().sort([("série", pymongo.ASCENDING)]):
        ...
        turmas.append({"curso": t['curso'], "série": t['série'], "ícone":ícones[t['curso']], "add": "enabled" if tAtual in usuárioDados['turmas'] or usuárioDados['turmas'] == ["*"] else "disabled", "matérias":t['matérias'], "id":ind})
        ind += 1
    return turmas

def calcularMatérias(db, usuario, turma=-1, turmas=-1):
    usuárioDados = db.Usuários.find({"usuário":usuario})[0]
    listaMat = []
    if turma != -1:
        for m in turma['matérias']:
            if m in usuárioDados['matérias'] or usuárioDados['matérias'] == ["*"]:
                listaMat.append(m)
    else:
        for turma in turmas:
            for m in turma['matérias']:
                if m in usuárioDados['matérias'] or usuárioDados['matérias'] == ["*"]:
                    if m not in listaMat:
                        listaMat.append(m)
    return listaMat
